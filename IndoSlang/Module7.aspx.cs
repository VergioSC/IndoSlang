...
namespace IndoSlang
{
    public partial class Module7 : System.Web.UI.Page
    {
        ...
        private int userId;
        private const int ModuleOrder = 7;
        private const int PreviousModuleOrder = 6;
        private const int PassingScore = 7;
        ...
        {
            int previousModuleId = GetModuleIdByOrder(PreviousModuleOrder);
            if (previousModuleId == 0) return;
            if (!HasCompletedModule(previousModuleId))
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            object parsedData;
                            try { parsedData = serializer.DeserializeObject(rawData); }
                            catch
                            {
        ...
        protected void btnSaveResult_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect(passed ? "~/Module8.aspx" : "~/Module7.aspx");
        }
